...
output_path<-"C:/Users/aslia/Desktop/pisi/ETH/master project/labbook/plots/01.19/190107_2773_biolector_sytox" #where to save the plots

OFFSET<-TRUE # decide if you want to offset all curves by their minimum measured value, such that all curves start around 0.

name_experimental_strain<- "TOR2(bk)"
...
biolector_data<- 
  as.data.frame(read.csv("190107_tor2_2773_sytox.csv")) %>% # read in the biolector file, from which the rows where the experiment is described is removed (so the first row is the column names)
...
biolector_data_strain<- 
  ...
  filter(Wells %in% c(6:24,30:48)) %>% # filter out the wells where a sample was taken, hence lid was pierced which causes a jump in the amplitude reading.
  
  mutate(Dose = case_when( Wells %in% seq(1,21,4) ~ 0,
                           Wells %in% seq(2,22,4) ~ 5,
                           Wells %in% seq(3,23,4) ~ 15,
                           Wells %in% seq(4,24,4) ~ 60,
                           Wells %in% seq(25,45,4) ~ 1,
                           Wells %in% seq(26,46,4) ~ 8,
                           Wells %in% seq(27,47,4) ~ 30,
                           Wells %in% seq(28,48,4) ~ 600,
                           Wells %in% c(29,37,45) ~ 600
...
doses_experiment_strain<-c(0,1,5,8,600)
...
colour_scale2<-scales::seq_gradient_pal("lightblue", "red", "Lab")(seq(0,1,length.out=8))
 ...
 f_plot(biolector_data_selected, c(0,NA),c(0,NA),colour_scale2) 
#   scale_x_continuous(limits = c(10,35), breaks = seq(10,35,8), labels=seq(10,35,8)-10) +
...
# ggsave("selected_data.jpeg",path = "C:/Users/aslia/Desktop/pisi/ETH/master project/labbook/paper/figures/titration/PMA1_sd_181213", height=6, width=9.04, units="cm")
# ggsave("selected_data.pdf",path = "C:/Users/aslia/Desktop/pisi/ETH/master project/labbook/paper/figures/titration/PMA1_sd_181213", height=6, width=9.04, units="cm")
```
