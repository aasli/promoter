...
output_path<-"C:/Users/aslia/Desktop/pisi/ETH/master project/labbook/paper/figures/titration/2772_ypd" #where to save the plots

OFFSET<-TRUE # decide if you want to offset all curves by their minimum measured value, such that all curves start around 0.

name_experimental_strain<- "tor2_gk"
...
biolector_data<- 
  as.data.frame(read.csv("181105_tor2_2772_ypd.csv")) %>% # read in the biolector file, from which the rows where the experiment is described is removed (so the first row is the column names)
...
doses_experiment_strain<-c(0,5,60,200,600)
...
f_plot(biolector_data_selected, c(10,25),c(0,NA),colour_scale2) +
