...
f_size_plot<-function(dataframe,fsc_column,ssc_column, xlab_title, ylab_title,
                      xlims,ylims){
  print(str(dataframe))
  dataframe<-as.data.frame(dataframe[which(dataframe[,6]=="0"),])
  ggplot() +
...
f_density_plot<-function(dataframe,labels,doses_column,size_column, citrine_column, 
                         legend_position,palette, x_breaks, legend_title, ncol_legend){
  
  final_plot<-ggplot()  
  dataframe<-as.data.frame(dataframe)
  doses<-unique(dataframe[,doses_column])
  labels<-labels[c(1,length(doses))]
  doses<-doses[c(1,length(doses))]
