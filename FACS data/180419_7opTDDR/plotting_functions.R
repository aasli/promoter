...
f_plot_sigmoid_curves<-
  ...
  function(fit_list,frame_list, control_list,control_list_sigmoid, ec_list, doses){
    ...
    descriptives[[1]][1,8]<-0.4
    
    final_plot<-
      final_plot + 
      geom_line(aes(x=descriptives[[1]][,8],y=descriptives[[1]][,1],colour=0)) +
    ...
    final_plot<-
      final_plot +
      geom_line(aes(x=c(experiment_doses),y=descriptives[[42]][24,1]), linetype="dashed")
