...
t0p1_file<-"180502_smalldose/6h"
t1p1_file<-"180502_smalldose/6h/controls"
pattern_read<-"Specimen_001"


output_path<-"C:/Users/aslia/Desktop/pisi/ETH/master project/labbook/plots/may18/180502_smalldose" # for saving plots
xlabel<-expression(paste("aTc[ng/mL]", sep = "")) 

starting_well<-1
wells_per_sample<-36

experiment_doses<- c(0,0.0000005,0.000001,0.000005,0.00001,0.00005,0.0001,0.0005,0.001,0.005,0.01,0.05,0.1,0.5,1,3,5,8,12,15,17,21,23,25,30,35,40,50,60,75,100,150,200,300,400)


columns_to_include<-c(2,5,7) #1=FSC-A, #2=FSC-H, #3=FSC-W, #4=SSC-A, #5=SSC-H, #6=SSC-W, #7= 488 [C]-H


strain_names<-c("2674","2759","2661","2683","70") 
time_points<-c("6h")

label_list<- c("7op final, NF",
               "7op final, NF+pRNR2-TetR-TUP1",
               "no repressor",
               "pTDH3",
               "fry70"
               )
...
lower_limit_fscw <-20000 #the value for subsetting based on FSC-W/H, lower limit
...
names_individual_histograms<-c("histogram_gridNF.jpeg","histogram_gridNF+TUP1.jpeg") # names for saving the plots
...
labels_to_use<-c(1,2)
colour_palette<-c("red","blue", "orange","green","black","pink") # define the colour palette for plotting
...
list_of_starting_points<-list(
  c(40000,0.8,20,1,300),
  c(40000,0.8,20,1,300)
)

time_point_sigmoid<-"2d"
height_sigmoid<-10
width_sigmoid<-15

## derivative of sigmoid fit

label_list_derivative<-c("NF","NF+TUP1")
#----------------------------------------------------------------------------------------------------------
...
f_histogram_induction<-function(dataframe1,dataframe2,doses){
  
    df1<-dataframe1[which(dataframe1[,4]==doses),]
    df1_0<-dataframe1[which(dataframe1[,4]==0),]
    
    df2<-dataframe2[which(dataframe2[,4]==doses),]
    df2_0<-dataframe2[which(dataframe2[,4]==0),]
    
    final_plot1<-ggplot() +
      geom_density(aes(x=df1[,3],colour=as.character(doses))) +
      geom_density(aes(x=df1_0[,3],colour="0")) +
      theme_bw() +
      ggtitle(paste("NF",doses))+
      theme(legend.position = "none") +
      xlim(0,15000)
    
    final_plot2<-ggplot() +
      geom_density(aes(x=df2[,3],colour=as.character(doses))) +
      geom_density(aes(x=df2_0[,3],colour="0")) +
      theme_bw() +
      ggtitle("NF+TUP1")+
      theme(legend.position = "none") +
      xlim(0,2000)
    
    grid<-cowplot::plot_grid(final_plot1,final_plot2)
    
    #f_save(grid,paste(doses,".jpeg"),output_path,"ind_histograms",10,15)
    return(grid)
...
induction_histograms<-lapply(experiment_doses[c(1:17)],f_histogram_induction,dataframe1=df_list[[1]],dataframe2=df_list[[2]])

for(i in seq(1,15,4)){
  
grid<-cowplot::plot_grid(plotlist = induction_histograms[c(i:(i+3))])

f_save(grid,paste(i,".jpeg"),output_path,"ind_histograms",10,15)
