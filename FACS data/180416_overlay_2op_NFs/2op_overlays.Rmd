...
t1p1_file<-"180327_2opypdsd/2op_p1/ypd"
t1p2_file<-"180327_2opypdsd/2op_p1/ypd/controls"

t1p3_file<-"180312_7op2op_ypd_sd/2op_ypd_sd/ypd"
t1p4_file<-"180312_7op2op_ypd_sd/2op_ypd_sd/ypd/controls"

t1p5_file<-"180322_2opypd/2opypd/overlay"
t1p6_file<-"180322_2opypd/2opypd/controls/overlay" # 4 controls

t1p7_file<-"170907_everything/t3_dose/overlay"
t1p8_file<-"170907_everything/t3_dose/controls" # 2 controls

t1p9_file<-"170920_everything/ypd_t1/overlay"
t1p10_file<-"170920_everything/ypd_t1/controls" # 2 control, only 1 fry70

t1p11_file<-"170926_media_doseR/ypd_t1/overlay"
t1p12_file<-"170926_media_doseR/ypd_t1/controls" # only 1 control, 2551
...
experiment_doses1<-c(0,0.2,1,3,5,6,8,10,13,15,17,20,23,26,30,35,40,45,50,65,80,100,200,400)
experiment_doses2<-c(0,0.2,1,3,5,6,8,10,13,15,17,20,23,26,30,35,40,45,50,65,80,100,200,400)
experiment_doses3<-c(0,0.2,0.5,1,1.5,2,2.5,3,4,5,6,7,8,9,10,12,15,18,25,35,60,100,200,400)
experiment_doses4<-(c(0,1,3,5,10,20,30,60,100,400,1000,3000))*2.085
experiment_doses<-list(experiment_doses1,experiment_doses2,experiment_doses3,experiment_doses4)

output_path<-"C:/Users/aslia/Desktop/pisi/ETH/master project/labbook/plots/april18/2op_overlay" # for saving plots
...
labels_to_use<-c(1,2)
colour_palette<-c("red","blue", "orange","green","black","grey","lightblue","pink") # define the colour palette for plotting
...
list_of_starting_points1<-list(
  c(30000,0.8,26,0.2,3000),
  c(30000,0.01,30,10,3000)
)

list_of_starting_points2<-list(
  c(30000,0.8,26,0.2,3000),
  c(30000,0.01,30,10,3000)
)

list_of_starting_points3<-list(
 c(30000,0.8,45,0.2,3000),
  c(30000,0.01,1,6,5000)
)

list_of_starting_points4<-list(
  c(30000,0.8,26,0.2,3000),
  c(30000,0.01,30,10,3000)
)
list_list_starting_points<-list(list_of_starting_points1,
                                list_of_starting_points2,
                                list_of_starting_points3,
                                list_of_starting_points4)
