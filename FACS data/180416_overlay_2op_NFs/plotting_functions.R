...
f_plot_sigmoid_curves<-
  ...
  function(fit_list,frame_list, control_list,control_list_sigmoid, ec_list_list, doses){
  
    
    final_plot<-ggplot() 
    
    for(k in c(1:length(fit_list))){
      fit<-fit_list[[k]]
      x_values_plotting<-x_values_plotting_list[[k]]
    # plot the lines
    for (i in c(1)){
      single_layer_line<- geom_line(aes_(x=x_values_plotting, y=fit[[i]],
                                         colour=paste(label_list_sigmoid[[i]],k)),size=1)
    ...
    frame<-frame_list[[k]]  
    for (j in c(1)){
      single_layer_point<- geom_point(aes_(x=frame[[j]][,8],y=frame[[j]][,1],
                                           colour=paste(label_list_sigmoid[[j]],k))) 
    ...
    breaks_sigmoid<-c(0.1,5,10,15,30,45,60,100,200,400)
    labels_x_axis<-c(0,5,10,15,30,45,60,100,200,400)
        pretty_plot<-final_plot +
...
f_spearman_plot<-function(variance_list,label_list){
  ...
  for(i in c(1:length(variance_list))){ #variance list calculated in the facs script.
    ...
    single_layer<-
      geom_point(aes_(x=experiment_dose, y=variances[[i]], colour=label_list[[i]])) 
    final_plot<-final_plot+single_layer
  }
