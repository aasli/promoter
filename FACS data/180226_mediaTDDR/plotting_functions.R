...
f_geom_histogram <- function(dataframe,control_sequence, dose_column, citrine_column, labels_histogram, 
  ...
  for(i in control_sequence) {
    ...
    doses_control<-unique(control[,dose_column])
    for(j in doses_control){
    single_layer<- geom_density(
      aes_(x=control[which(control[,dose_column]==j),citrine_column],
...
f_plot_sigmoid_curves<-
  ...
  function(fit_list,frame_list){
    ...
    # plot the lines
    for (i in c(1:2)){
      single_layer_line<- geom_line(aes_(x=x_values_eth, y=fit_list[[i]],
                                         colour=label_list_sigmoid[[i]]),size=1)
      final_plot<-final_plot+single_layer_line
    }
    
    for (i in c(3:4)){
      single_layer_line<- geom_line(aes_(x=x_values_pro, y=fit_list[[i]],
                                         colour=label_list_sigmoid[[i]]),size=1)
      final_plot<-final_plot+single_layer_line
    }
