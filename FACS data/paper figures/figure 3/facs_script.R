...
histograms<-mapply(f_geom_histogram,histogram_df_list,
...
names(histograms)<-histogram_strain_names
### making the grids


name_list_histogram<-histogram_strain_names # should contain one name per strain that you want
#to create a grid for.
label_list_histogram<-histogram_label_list # should contain one label per grid, 
...
density_plots<-lapply(df_with_size[dataframes_to_use],f_density_plot,
                      labels=labels_size_vs_induction, 
...
name_list_density<- strain_names[dataframes_to_use] # should contain one name per strain 
#that you want to create a grid for.
label_list_density<-label_list[dataframes_to_use] # should contain one label per grid, 
#and the order of the labels must correspond to the order in which the strains appear in name_list.
...
mapply(f_save,density_plots,names(grid_plots_density),
...
f_save(sigmoid_plot,paste("sigmoid_fit_weighted_tup1.jpeg",time_point_sigmoid,sep = "_"),
       output_folder=output_path,output_path="", 
