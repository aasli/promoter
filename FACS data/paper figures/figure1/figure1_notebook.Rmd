...
f_plot_sigmoid_curves_single<-
  ...
  function(fit_list,frame_list, control_frame_list,dose_control,label_list_control, xlim, ylim,
    ...
    for(k in c(1:length(control_frame_list))){
     ...
     # plot individual density estimates
    # for(m in c(1:length(frame_list))){
    #   
    #   density_layer<-geom_density(aes_(x=frame_list[[m]], ))
    #   
    # }
...
#define x axis doses and the labels for plotting
doses_experiment<-c(0.1,1,2,3,5,10,20,30,60,100)
labels_x_axis<- c("0","1","2","3","5","10","20","30","60","100")
...
```{r}
deneme<-cbind(dnorm(c(1,10)),c(rep(1,10),rep(2,10)))
deneme
library(ggridges)
ggplot() +
  geom_violin(aes_(y=deneme[,1], x=deneme[,2], group=deneme[,2])) 
