...
f_spearman_plot<-function(variance_list,label_list,title){
  
  final_plot<- ggplot() 
  
  for(i in c(1:length(variance_list))){ #variance list calculated in the facs script.
    single_layer<-
      geom_point(aes_(x=experiment_doses, y=variance_list[[i]], colour=label_list[i])) 
    final_plot<-final_plot+single_layer
  }
...
## for seeing sizes for each dose

f_individual_size<-function(dataframe){
  
  plot<-ggplot(dataframe) +
    geom_point(aes(x=dataframe[,1],y=dataframe[,2]),size=0.3) +
    facet_wrap(~Dose) +
    theme_bw() +
...
#individual_size_plots<-f_individual_size(df_list[[1]])

#f_save(individual_size_plots,"size_grid.jpeg",output_path,"",10,15)
