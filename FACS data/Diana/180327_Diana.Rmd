...
output_path<-"C:/Users/aslia/Desktop/pisi/ETH/master project/labbook/plots/Diana" # for saving plots
xlabel<-expression(paste(beta,"-estradiol [nM]", sep = "")) 

starting_well<-1
wells_per_sample<-12

experiment_doses<- c(0,1,3,8,22,31,60,125,200,500,1000,2000)


columns_to_include<-c(3,6,8) #1=FSC-A, #2=FSC-H, #3=FSC-W, #4=SSC-A, #5=SSC-H, #6=SSC-W, #7= 488 [C]-H


strain_names<-c("DIana") 
time_points<-c("6h")

label_list<- c("Diana TF, SDC"
               )
...
labels_histogram<-c("0","1","3","8","22","31","60","125","200","500","1000","2000",
...
## sigmoid fits
descriptives_to_use_sigmoid<-c(1)
labels_to_use<-c(1)
...
label_list_derivative<-c("Diana TF")
#----------------------------------------------------------------------------------------------------------
...
legend_position_density<-c(0.8,0.68)
size_column<-5
citrine_column<-3
dose_column<-4

doses_size_vs_induction<-c(0,1,3,8,22) # the doses you want to plot. if you change this, you should change the manual colour scale inside the function as well. 
