...
mapply(f_save,histograms,names(histograms),
...
density_plots<-lapply(df_with_size,f_density_plot, labels=labels_histogram, 
...
mapply(f_save,density_plots,"density_start",
...
spearman<-f_spearman_plot(variances,label_list_spearman, "size vs fluorescence")


f_save(spearman,"spearman.jpeg",output_path, "",10,15)


## for spearman corellation between FSC-W vs SSC-W
variances_size<-vector("list",length(spearman_list))

for(k in c(1:length(spearman_list))){
  variance<-vector()
  for(i in experiment_doses){
    dataframe<-spearman_list[[k]][which(spearman_list[[k]][,4]==i),]
    var<-cor(dataframe[,1],dataframe[,2], method = "spearman")
    variance<-c(variance,var)
    
  }
  variances_size[[k]]<-variance
}

spearman_size<-f_spearman_plot(variances_size,label_list_spearman, "FSC-W vs SSC-W")


f_save(spearman_size,"spearman_size.jpeg",output_path, "",10,15)
