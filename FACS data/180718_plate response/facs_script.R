...
df_list<-c(f_df_list(t0p1,starting_well,10,experiment_doses_ypd,columns_to_include),
           f_df_list(t0p2,starting_well,9,experiment_doses_sd,columns_to_include)
                     )
...
descriptives<-mapply(f_descriptives,df_list,dose_descriptives, 
                       MoreArgs = list(column=cols_descriptives), SIMPLIFY = F)
...
histograms<-mapply(f_geom_histogram,df_list,control_sequence,
                   MoreArgs = list(dose_column,
                                   ...
                                   ylimits_histogram, df_list, label_list),
...
control_list_sigmoid<-label_list[c(2,3,4)]
