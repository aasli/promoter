...
f_geom_histogram <- function(dataframe, dose_column, citrine_column,
                             labels_histogram,doses_histogram,size,breaks,legend_title,legend_ncol,
                             legend_position,xlimits,ylimits) {
...
f_plot_sigmoid_curves<-
  ...
  function(fit_list,frame_list){
    ...
    pretty_plot<-final_plot +
      ...
      scale_x_log10(breaks = c(0.1,doses_experiment[c(2:length(doses_experiment))]),
